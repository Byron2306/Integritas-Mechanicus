...
class MandosContextService:
    ...
    def _restore_sealed_state_if_needed(self, svc: Any) -> None:
        ...
        try:
            data_root = _mandos_data_root()
            covenant_dir = data_root / "covenants" / "constitutional"
            principal_dir = data_root / "principal"
    ...
    def _build_learner_history_profile(self, encounters: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Compress longer learner history into adaptive pedagogy signals."""
        payloads = [enc.get("payload", enc) for enc in encounters or []]
        if not payloads:
            return {
                "sample_size": 0,
                "dominant_challenge": "unknown",
                "recurring_scaffolds": [],
                "handback_rate": 0.0,
                "retrieval_use_rate": 0.0,
                "adaptation_hint": "No learner history available; begin with baseline diagnosis.",
            }
        ...
        challenge_counts = counts("challenge_type")
        dominant_challenge = max(challenge_counts, key=challenge_counts.get) if challenge_counts else "unknown"
        scaffold_counts: Dict[str, int] = {}
        for payload in payloads:
            for scaffold in payload.get("scaffolds_applied", []) or payload.get("scaffolds", []) or []:
                scaffold_counts[str(scaffold)] = scaffold_counts.get(str(scaffold), 0) + 1
        recurring_scaffolds = [
            name for name, _ in sorted(scaffold_counts.items(), key=lambda item: item[1], reverse=True)[:4]
        ]
        handback_hits = sum(
            1
            for payload in payloads
            if payload.get("speech_act") == "handback"
            or payload.get("handback_reason")
            or payload.get("pedagogical_mediation_passed")
        )
        retrieval_hits = sum(1 for payload in payloads if payload.get("retrieval_triggered") or payload.get("retrieval_used"))
        lawful_hits = sum(1 for payload in payloads if payload.get("criterion_overall") == "LAWFUL")
        total = max(1, len(payloads))
        if dominant_challenge in {"KNOWLEDGE_GAP", "DOMAIN_TRANSFER", "EPISTEMIC_OVERREACH"}:
            hint = "Lead with source-grounded scaffolding and ask the learner to state claim/evidence/warrant."
        elif handback_hits / total < 0.5:
            hint = "Increase agency restoration: end with one learner-authored next move."
        else:
            hint = "Maintain compact mediation and gradually fade scaffolds toward learner autonomy."
        return {
            "sample_size": len(payloads),
            "dominant_challenge": dominant_challenge,
            "challenge_counts": challenge_counts,
            "recurring_scaffolds": recurring_scaffolds,
            "handback_rate": round(handback_hits / total, 3),
            "retrieval_use_rate": round(retrieval_hits / total, 3),
            "lawful_rate": round(lawful_hits / total, 3),
            "adaptation_hint": hint,
        }

    def _summarize_learner_history_profile(self, profile: Optional[Dict[str, Any]]) -> str:
        if not profile:
            return "None"
        return (
            f"sample={profile.get('sample_size', 0)} | "
            f"dominant challenge={profile.get('dominant_challenge', 'unknown')} | "
            f"handback rate={profile.get('handback_rate', 0)} | "
            f"retrieval use={profile.get('retrieval_use_rate', 0)} | "
            f"lawful rate={profile.get('lawful_rate', 0)} | "
            f"recurring scaffolds={', '.join(profile.get('recurring_scaffolds') or []) or 'none'} | "
            f"adaptation={profile.get('adaptation_hint', 'baseline first')}"
        )
    ...
    def _is_minimal_operational_query(self, current_topic: str) -> bool:
        topic = (current_topic or "").strip().lower()
        if not topic:
            return False
        if len(topic.split()) > 18:
            return False
        markers = (
            "status",
            "operational",
    ...
    def _is_plain_greeting(self, current_topic: str) -> bool:
        topic = " ".join((current_topic or "").strip().lower().split())
    ...
    def _derive_world_event_state(
        ...
        plain_greeting = self._is_plain_greeting(topic)
        reflective_markers = (
            "overwhelmed",
            "judging myself",
        ...
        elif casual and (reentry_state or top_thread) and not plain_greeting:
        ...
        return {
            ...
            "routing_directives": {
                ...
                "forbid_generic_greeting": bool(casual and (reentry_state or top_thread) and not plain_greeting),
                "ask_one_short_followup": bool(casual and (reentry_state or top_thread) and not plain_greeting),
    ...
    async def build_context(
        ...
        try:
            ...
            ctx.learner_history_profile = self._build_learner_history_profile(encounters)
        ...
        if self._is_casual_reentry(current_topic) and ctx.reentry_state and not self._is_plain_greeting(current_topic):
        ...
        if self._is_minimal_operational_query(current_topic):
            ctx.active_office = "speculum"
            ctx.casual_reentry_active = False
            ctx.world_event_state = None
            if ctx.response_parameters:
                ctx.response_parameters["active_office"] = "speculum"
    ...
    def to_system_prompt(self, ctx: PreResponseContext) -> str:
        ...
        minimal_operational = self._is_minimal_operational_query(ctx.current_topic)
        ...
        if minimal_operational:
            band_c = [
                "### BAND C: PRINCIPAL CONTEXT (MINIMAL) ###",
                f"Principal: {ctx.principal_name or 'Anonymous'}",
                "Use principal context only for address and boundary alignment. Do not foreground biography unless explicitly asked.",
                "",
            ]
        else:
            band_c = [
        ...
        learner_history_band = [
            "",
            "### BAND E2: LEARNER HISTORY ADAPTATION ###",
            self._summarize_learner_history_profile(ctx.learner_history_profile),
            "Use this as a developmental pattern, not as a fixed identity. Adapt scaffolding, source demands, and handback intensity to the pattern.",
        ]
        relational_band = [
            "",
        ...
        full_prompt = band_a + band_b + band_c + zpd_lines + encounter_band + learner_history_band + relational_band
        return "\n".join(full_prompt)
